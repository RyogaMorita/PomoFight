...
**200超えたらどうなる / どうするか:**
1. マッチングが繋がらない・対戦が始まらないユーザーが出る
2. Supabase ダッシュボード → Reports → Realtime で現在の接続数を確認
3. まずコードに `unsubscribe()` 漏れがないか確認（再起動しても減らない場合はリーク）
4. 即時対処: Supabase ダッシュボード → Realtime → 不要な接続を強制切断
5. 根本対処: Pro プラン（$25/月）にアップグレード → 上限 500 に拡張
6. 将来対処: Realtime を使わず DB ポーリング（5秒ごとに SELECT）に切り替えると接続数0で済む
